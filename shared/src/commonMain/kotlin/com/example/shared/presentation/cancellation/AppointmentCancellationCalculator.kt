...
object AppointmentCancellationCalculator {
  ...
  fun buildClientPreview(
    ...
    val minutesUntilAppointment =
        minutesBetween(start = currentDateTime, end = appointment.serviceStartAt)

    val resolved =
        resolveClientRefundRule(
  ...
  fun buildWorkerPreview(appointment: Appointment): AppointmentCancellationPreview {
    return AppointmentCancellationPreview(
  ...
  private data class RefundRule(val label: String, val percentage: Int)
  ...
  private fun resolveClientRefundRule(
    ...
    return when {
      minutesUntilAppointment >= MINUTES_7_DAYS -> {
        RefundRule(label = "7 días o más antes", percentage = safePolicy.before7DaysOrMore)
      }

      minutesUntilAppointment >= MINUTES_3_DAYS -> {
        RefundRule(label = "Entre 3 y 6 días", percentage = safePolicy.between3And6Days)
      }

      minutesUntilAppointment >= MINUTES_48_HOURS -> {
        RefundRule(label = "48 horas antes", percentage = safePolicy.before48h)
      }

      minutesUntilAppointment >= MINUTES_24_HOURS -> {
        RefundRule(label = "24 horas antes", percentage = safePolicy.before24h)
      }

      else -> {
        RefundRule(label = "Mismo día o menos de 24h", percentage = safePolicy.sameDayOrLess24h)
      }
    }
  }
  ...
  private fun toAbsoluteMinutes(dateTime: SimpleDateTime): Long {
    val days = daysFromCivil(year = dateTime.year, month = dateTime.month, day = dateTime.day)
