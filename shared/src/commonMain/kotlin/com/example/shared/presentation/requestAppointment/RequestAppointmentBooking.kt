...
private data class BusyAppointmentRange(val startMinutes: Int, val endMinutes: Int)
...
fun hasUsableCurrentTime(snapshot: CurrentTimeSnapshot): Boolean {
  ...
  val hasExplicitDate =
      snapshot.todayYear in 1..9999 && snapshot.todayMonth in 1..12 && snapshot.todayDay in 1..31
...
fun buildAvailableDays(
  ...
  val monthEnd = lastDayOfMonth(year = monthStart.year, monthNumber = monthStart.monthNumber)

  val startDate =
      if (monthStart.year == today.year && monthStart.monthNumber == today.monthNumber) {
        today
      } else {
        monthStart
      }
...
fun buildAvailableTimes(
  ...
  val busyRanges = buildBusyRangesForDate(date = date, appointments = existingAppointments)

  val existingServiceStarts =
  ...
  validBlocks.forEach { block ->
    ...
    while (cursor < blockEnd) {
      ...
      val fitsInsideWorkerSchedule = serviceStart >= blockStart && serviceEnd <= blockEnd

      val hasEarlierAppointment = existingServiceStarts.any { it < serviceStart }

      val effectiveBufferBefore =
          if (hasEarlierAppointment) {
            requestedTravel
          } else {
            0
          }
...
fun fullDateTimeLabel(date: LocalDate, time: AppointmentTimeOption): String {
...
fun buildPendingAppointment(
  ...
  val serviceStartEpochMillis = toAppointmentEpochMillis(date = selectedDate, time = selectedTime)
  ...
  val blockedStartEpochMillis = serviceStartEpochMillis - (effectiveBufferBefore * 60_000L)

  val blockedEndEpochMillis = serviceEndEpochMillis

  val serviceStartDateTime =
      Instant.fromEpochMilliseconds(serviceStartEpochMillis).toLocalDateTime(APPOINTMENT_TIME_ZONE)

  val serviceEndDateTime =
      Instant.fromEpochMilliseconds(serviceEndEpochMillis).toLocalDateTime(APPOINTMENT_TIME_ZONE)

  val blockedStartDateTime =
      Instant.fromEpochMilliseconds(blockedStartEpochMillis).toLocalDateTime(APPOINTMENT_TIME_ZONE)

  val blockedEndDateTime =
      Instant.fromEpochMilliseconds(blockedEndEpochMillis).toLocalDateTime(APPOINTMENT_TIME_ZONE)

  val now = safeEpochMillis(snapshot = draft.currentTime, fallback = serviceStartEpochMillis)
...
private fun snapshotLocalDateTime(snapshot: CurrentTimeSnapshot): LocalDateTime? {
  ...
  return Instant.fromEpochMilliseconds(millis).toLocalDateTime(APPOINTMENT_TIME_ZONE)
}
...
private fun workerHasScheduleForDate(date: LocalDate, schedule: List<WorkerSchedule>): Boolean {
  val scheduleForDay = findScheduleForDate(date, schedule) ?: return false
  return scheduleForDay.enabled &&
      scheduleForDay.timeBlocks.any { it.start.isNotBlank() && it.end.isNotBlank() }
}

private fun safeEpochMillis(snapshot: CurrentTimeSnapshot, fallback: Long): Long {
  ...
  val serviceAppointments =
      appointments
          ...
          .mapNotNull { appointment ->
            val serviceStart =
                resolveAppointmentServiceStartMinutes(appointment) ?: return@mapNotNull null
            val serviceEnd =
                resolveAppointmentServiceEndMinutes(appointment, serviceStart)
...
private fun resolveAppointmentServiceStartMinutes(appointment: Appointment): Int? {
  return when {
...
private fun resolveAppointmentServiceEndMinutes(appointment: Appointment, startMinutes: Int): Int? {
...
private fun appointmentBelongsToDate(appointment: Appointment, selectedDate: LocalDate): Boolean {
...
private fun findScheduleForDate(date: LocalDate, schedule: List<WorkerSchedule>): WorkerSchedule? {
  ...
  return schedule.firstOrNull {
    it.enabled &&
        (it.dayKey.trim().lowercase() == expectedDayKey || it.dayNumber == expectedDayNumber)
  }
}
...
private fun topDayLabel(date: LocalDate, today: LocalDate): String {
  return when {
    date == today -> "HOY"
    else ->
        when (date.dayOfWeek) {
...
private fun toAppointmentEpochMillis(date: LocalDate, time: AppointmentTimeOption): Long {
  val localDateTime =
      LocalDateTime(
          year = date.year,
  ...
  return localDateTime.toInstant(APPOINTMENT_TIME_ZONE).toEpochMilliseconds()
}
...
private fun epochMillisToIsoLocalDateTime(epochMillis: Long): String {
  ...
  val localDateTime =
      Instant.fromEpochMilliseconds(epochMillis).toLocalDateTime(APPOINTMENT_TIME_ZONE)
...
private fun lastDayOfMonth(year: Int, monthNumber: Int): LocalDate {
  val firstDay = LocalDate(year = year, monthNumber = monthNumber, dayOfMonth = 1)
...
private fun fullMonthLabel(year: Int, monthNumber: Int): String {
  val month =
      when (monthNumber) {
        1 -> "enero"
        2 -> "febrero"
