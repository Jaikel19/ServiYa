...
class BookingRepository(private val remote: IRemoteBookingDataSource) : IBookingRepository {

  override fun getBookingsByWorker(workerId: String): Flow<List<Booking>> =
      remote.getBookingsByWorker(workerId).catchEmpty("fetching bookings by worker")

  override fun getBookingsByClient(clientId: String): Flow<List<Booking>> =
      remote.getBookingsByClient(clientId).catchEmpty("fetching bookings by client")
