...
class RemoteBookingDataSource : IRemoteBookingDataSource {
  ...
  override suspend fun getBookingById(bookingId: String): Booking? {
    return try {
      val doc = db.collection("appointments").document(bookingId).get()
  ...
  override suspend fun getWorkerProfile(workerId: String): WorkerProfile? {
    return try {
      val doc = db.collection("users").document(workerId).get()
  ...
  override suspend fun confirmPayment(bookingId: String) {
    db.collection("appointments").document(bookingId).update("status" to "confirmed")
  }

  override suspend fun approvedAppointment(bookingId: String) {
    db.collection("appointments").document(bookingId).update("status" to "approved")
  }

  override suspend fun startAppointment(bookingId: String) {
    db.collection("appointments").document(bookingId).update("status" to "in_progress")
  }

  override suspend fun completeAppointment(bookingId: String) {
    db.collection("appointments").document(bookingId).update("status" to "completed")
  }
