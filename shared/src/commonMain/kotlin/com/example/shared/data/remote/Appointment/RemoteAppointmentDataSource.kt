...
class RemoteAppointmentDataSource : IRemoteAppointmentDataSource {
  ...
  override suspend fun getAppointmentById(appointmentId: String): Flow<Appointment?> {
    return db.collection(appointmentsCollection).document(appointmentId).snapshots.map { doc ->
      try {
  ...
  override suspend fun updateAppointment(appointment: Appointment) {
    try {
      db.collection(appointmentsCollection).document(appointment.id).set(appointment)
    } catch (e: Exception) {
  ...
  override suspend fun deleteAppointment(appointmentId: String) {
    try {
      db.collection(appointmentsCollection).document(appointmentId).delete()
    } catch (e: Exception) {
  ...
  override suspend fun approveAppointment(appointmentId: String) {
    db.collection(appointmentsCollection)
        ...
        .update("status" to "approved", "updatedAt" to currentDateTimeString())
  }

  override suspend fun confirmPayment(appointmentId: String) {
    db.collection(appointmentsCollection)
        .document(appointmentId)
        .update("status" to "confirmed", "updatedAt" to currentDateTimeString())
  }

  override suspend fun startAppointment(appointmentId: String) {
    db.collection(appointmentsCollection)
        .document(appointmentId)
        .update("status" to "in_progress", "updatedAt" to currentDateTimeString())
  }

  override suspend fun completeAppointment(appointmentId: String) {
    db.collection(appointmentsCollection)
        .document(appointmentId)
        .update("status" to "completed", "updatedAt" to currentDateTimeString())
  }
  ...
  override suspend fun markPaymentPending(appointmentId: String) {
    try {
      ...
      println("ERROR markPaymentPending: ${e.message}")
    }
  }
}
